...
from pypokerai.value_function import MLPFiveLayerScaledScalarFeaturesValueFunction
...
class ApproxActionValueFunction(SarsaApproxActionValueFunction):
    ...
    def setup(self):
        self.delegate = MLPFiveLayerScaledScalarFeaturesValueFunction(blind_structure, self._handicappers)
        self.delegate.setup()
...
TRAINING_TITLE = "mlp_5layer_%s" % time_stamp
...
task = TexasHoldemTask(final_round=1000, scale_reward=True, lose_penalty=False, shuffle_position=True, action_record=True)
task.set_opponent_value_functions([value_func]*9)
policy = EpsilonGreedyPolicy(eps=1.0)
policy.set_eps_annealing(1.0, 0.1, int(TEST_LENGTH*0.8))
