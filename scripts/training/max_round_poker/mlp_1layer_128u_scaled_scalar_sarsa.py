...
class ApproxActionValueFunction(SarsaApproxActionValueFunction):
    ...
    def setup(self):
        self.delegate = MLPOneLayerScaledScalarFeaturesValueFunction(NB_UNIT, blind_structure, self._handicappers)
        self.delegate.setup()
...
TRAINING_TITLE = "mlp_1layer_%du_scaled_scalar_feature_sarsa_trash_%s" % (NB_UNIT, time_stamp)
...
task = TexasHoldemTask(final_round=1000, scale_reward=True, lose_penalty=False, shuffle_position=True)
